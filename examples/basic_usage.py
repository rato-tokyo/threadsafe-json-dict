...
def main():
    ...
    # JSON保存機能付き辞書を作成
    with ThreadSafeJsonDict("output/data.json") as data:
        print("=== 基本的な使用例 ===")

        # 辞書ライクな設定
        data["user_info"] = {"name": "Alice", "age": 30, "email": "alice@example.com"}
        data["settings"] = {"theme": "dark", "notifications": True, "language": "ja"}
        ...
        # 保存先を変更して別ファイルに保存
        data.set_json_file_path("output/updated_data.json")
        data.save()

        # 保存されたファイルの内容を確認
        print("\n=== 保存されたJSONファイルの内容 ===")
        try:
            with open("output/updated_data.json", encoding="utf-8") as f:
                print(f.read())
...
def demo_load_save():
    ...
    # 新しい辞書を作成
    with ThreadSafeJsonDict("demo_data.json") as data1:
        # データを設定して保存
    ...
    # 別の辞書インスタンスで読み込み
    with ThreadSafeJsonDict("demo_data_v1.1.json") as data2:
...
def test_issue_report_scenario():
    ...
    # テストデータ作成
    with ThreadSafeJsonDict("test_cache.json") as data:
        data["key1"] = "value1"
    ...
    # 反復処理テスト（問題レポートのシナリオ）
    with ThreadSafeJsonDict("test_cache.json") as status_dict:
        status_dict.load()
        try:
            count = 0
            print("=== items() 反復処理テスト ===")
